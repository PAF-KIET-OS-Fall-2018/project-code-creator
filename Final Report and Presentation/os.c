    ...
    for (i=0;i<length;i++)
    {
        switch (inputBuffer[i])
        {
            ...
            default :           
                ...
                if (inputBuffer[i] == '&')
                {
                    *background  = 1; //this background is the differentiate whether the child process is invoked in background
                    inputBuffer[i] = '\0';
                }
        }
    }
...
int main(void)
{
    ...
    pid_t pid,tpid;
    int i;
   signal(SIGINT,siggnal);//show history on ctlr+C 	
    
    while (1) //infinite loop for shell prompt
    {            
        background = 0; //background =0 by default
	...
	fflush(stdout);
       ...
       if(-1!=setup(inputBuffer,args,&background)) // get next command  
	{
		pid = fork();
        	if (pid < 0)//if pid is less than 0, forking fails
        	{
       		 ...
       		 else if (pid == 0)//if pid ==0 child
        	{
            
           	 	//Wrong command and exit command
